...
RawClusterBuilderTemplateSub::~RawClusterBuilderTemplateSub()
{
  delete bemc;
}

RawClusterBuilderTemplateSub::RawClusterBuilderTemplateSub(const std::string &name)
  : SubsysReco(name)
  , _clusters(nullptr)
    //, _min_tower_e(0.020)
    // DVP HACK
...
int RawClusterBuilderTemplateSub::InitRun(PHCompositeNode *topNode)
{
  try
  {
    CreateNodes(topNode);
...
bool RawClusterBuilderTemplateSub::Cell2Abs(RawTowerGeomContainer *towergeom, float phiC, float etaC, float &phi, float &eta)
{
  ...
  if (iphi < 0 || iphi >= NPHI)
  {
    printf("RawClusterBuilderTemplateSub::Cell2Abs: wrong input phi: %d\n", iphi);
    return false;
  }
  ...
  if (ieta < 0 || ieta >= NETA)
  {
    printf("RawClusterBuilderTemplateSub::Cell2Abs: wrong input eta: %d\n", ieta);
    return false;
  }
...
int RawClusterBuilderTemplateSub::process_event(PHCompositeNode *topNode)
{

  std::cout << " DVP : RawClusterBuilderTemplateSub::process_event " << std::endl;

  string towernodename = "TOWER_CALIB_CEMC_SUB1";
  // Grab the towers
 ...
 */
  ...
  std::cout << " DVP : _min_tower_e = " << _min_tower_e << std::endl;
  ...
  for (; itr != begin_end.second; ++itr)
  {
    ...
    if (tower->get_energy() > _min_tower_e)
    {
      ...
      //std::cout << " DVP : entry at ix / iy = " << ix << " / " << iy << " (ich = " << ich << " ), amp = " << tower->get_energy() << " * fEnergyNorm = " << fEnergyNorm << ", HitList now at size = " << HitList.size() << std::endl;
    }
  }
  ...
  //std::cout << " DVP : size of ClusterList = " << ClusterList->size() << std::endl;
  ...
  for (pc = ClusterList->begin(); pc != ClusterList->end(); ++pc)
  {
    ...
    if ( pc->GetTotalEnergy() < 1 ) continue;
...
bool RawClusterBuilderTemplateSub::CorrectPhi(RawCluster *cluster, RawTowerContainer *towers, RawTowerGeomContainer *towergeom)
{
...
int RawClusterBuilderTemplateSub::End(PHCompositeNode *topNode)
{
  return Fun4AllReturnCodes::EVENT_OK;
}

void RawClusterBuilderTemplateSub::CreateNodes(PHCompositeNode *topNode)
{
  ...
  ClusterNodeName = "CLUSTER_CEMC_SUB";
